...
import type { MongoCommonNameInfo } from "./dtos.js";


export async function getAutomationAccountsByProperties({csAccountId, accountName, locationId}: { csAccountId?: number; accountName?: string; locationId?: string; }): Promise<MongoCommonNameInfo> {
    const result: MongoCommonNameInfo = {};

    await ovrcMongo.withMongo(async (wholeDB) => {
...
export async function getAutomationAccountsByCommonName(commonNameOrMac: string): Promise<MongoCommonNameInfo> {
    return ovrcMongo.withMongo(async (wholeDB) => {
        const result: MongoCommonNameInfo = {};
        const db = wholeDB.db("ovrcmain");
        console.log("Connected to MongoDB");

        console.log("Getting automation accounts for common name or mac", commonNameOrMac);

        const mac = commonNameOrMac.split('_').slice(-1)[0].replaceAll(':', '').match(/../g).join(":");

        console.log("Converted common name or mac to mac address", mac);
...
export async function getAutomationAccountsByCommonNameOld(commonName: string): Promise<MongoCommonNameInfo> {
    return ovrcMongo.withMongo(async (wholeDB) => {
        const result: MongoCommonNameInfo = {};
        const db = wholeDB.db("ovrcmain");
        console.log("Connected to MongoDB", db);
