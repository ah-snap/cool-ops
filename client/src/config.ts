...
export const FORWARDS_BASE_URL = normalize(
	env.VITE_FORWARDS_BASE_URL || env.REACT_APP_FORWARDS_BASE_URL || "http://localhost:3004/api"
);
...
export const FORWARDS_SOCKET_BASE_URL = normalize(
	env.VITE_FORWARDS_SOCKET_BASE_URL || env.REACT_APP_FORWARDS_SOCKET_BASE_URL || stripApiSuffix(FORWARDS_BASE_URL)
);

export const apiUrl = (path: string): string => `${API_BASE_URL}${path.startsWith("/") ? path : `/${path}`}`;
export const forwardsUrl = (path: string): string => `${FORWARDS_BASE_URL}${path.startsWith("/") ? path : `/${path}`}`;
