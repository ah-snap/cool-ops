...
export default function DCodeValidationPage() {
  ...
  const nextButtonClicked = () => {
    ...
    rows.forEach((row) => {
      ...
      const dCodesFromSecurity16 = row.dCodesString.split(",").map((d) => d.trim());
    ...
    const nextOutput = stateOrder.map((state) => rows.filter((row) => row.state === state).map((row) => row.output).join("\n")).join("\n\n");

    setOutput(nextOutput);
  };

  return <PageShell containerClassName="dcodeValidationPage">
